...
def convert_dif(package_json: str) -> str:
    """Returns XML formatted string compatible with GCMD DIF 10.2 format.
    ...
    try:
        ...
        converted_dict = dif_convert_dataset(package_dict)  # Convert package to OrderedDict
...
def dif_convert_dataset(dataset_dict: dict):
    ...
    dif_metadata_dict['Dataset_Citation']['Dataset_Publisher'] = json.loads(
        dataset_dict.get('publication', '{}')).get('publisher', '')
    ...
    dif_metadata_dict['Dataset_Citation']['Data_Presentation_Form'] = ','.join(get_resource_formats(dataset_dict))
    ...
    dif_metadata_dict['Personnel']['Contact_Person']['First_Name'] = maintainer.get('given_name',
                                                                                    maintainer.get('name', '')
                                                                                    .strip()
                                                                                    .split(' ')[0]).strip()
    dif_metadata_dict['Personnel']['Contact_Person']['Last_Name'] = maintainer.get('name', '').strip().split(' ')[
        -1]
    ...
    if spatial:
        ...
        if spatial.get('type') == 'Point':
            bound_box_coordinates = get_bounding_rectangle(spatial.get('coordinates', []))
            ...
            dif_metadata_dict['Spatial_Coverage']['Geometry']['Polygon']['Center_Point'] = copy.deepcopy(
                dif_metadata_dict['Spatial_Coverage']['Geometry']['Bounding_Rectangle']['Center_Point'])
    ...
    dif_metadata_dict['Use_Constraints'] = \
        'Usage constraints defined by the license "' + license.strip() + '", see ' + license_url

    # Dataset_Language
    dif_metadata_dict['Dataset_Language'] = get_dif_language_code(dataset_dict.get('language', 'en'))
    ...
    dif_metadata_dict['Organization']['Organization_Name'][
        'Long_Name'] = 'Swiss Federal Institute for Forest, Snow and Landscape Research WSL'
    ...
    # if purpose:
    #     dif_metadata_dict['Summary']['Purpose'] = get_or_missing(extras_dict, 'purpose', ignore_case=True)
...
def get_science_keywords(data_dict, extras_dict):
    ...
    custom_keywords = get_ignore_case(extras_dict, 'science_keywords').upper().split(',')
    ...
    organizations_keywords_dict = {
        'biodiversity-and-conservation-biology': ['EARTH SCIENCE', 'BIOSPHERE', 'ECOLOGICAL DYNAMICS',
                                                  'COMMUNITY DYNAMICS', 'BIODIVERSITY FUNCTIONS'],
        ...
        'plant-animal-interactions': ['EARTH SCIENCE', 'BIOSPHERE', 'ECOLOGICAL DYNAMICS',
...
def get_bounding_rectangle_dict(spatial_dict):
...
def is_counter_clockwise(points):
    ...
    try:
        ...
        log.error("Unexpected error converting to float (is_counter_clockwise):", sys.exc_info()[0])
