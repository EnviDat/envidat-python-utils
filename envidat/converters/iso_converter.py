...
from envidat.api.v1 import get_package, get_protocol_and_domain, get_metadata_list_with_resources
...
def iso_convert_dataset(dataset_dict: dict):
    ...
    rpc_ci_contact['gmd:address']['gmd:CI_Address']['gmd:electronicMailAddress'] = get_or_missing(maintainer, 'email')
    ...
    citation_dict['gmd:date']['gmd:CI_Date']['gmd:date'] = {'gco:Date': get_publication_date(dataset_dict)}
    ...
    if access_constraints:
        md_legal_constraints['gmd:accessConstraints'] = {
            'gmd:MD_RestrictionCode': {'@codeListValue': cap_code(access_constraints),
    ...
    if use_constraints:
        md_legal_constraints['gmd:useConstraints'] = {
            'gmd:MD_RestrictionCode': {'@codeListValue': cap_code(use_constraints),
                                       '@codeList': 'http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_RestrictionCode'}}
    md_legal_constraints['gmd:otherConstraints'] = get_or_missing(dataset_dict, 'license_title')
    ...
    if category:
        md_data_id['gmd:topicCategory'] = {'gmd:MD_TopicCategoryCode': cap_code(category)}
    ...
    # transfer options
    online_resources = []
    online_resource_dataset = get_online_resource(package_url, 'dataset metadata', 'information')
...
def get_publication_date(data_dict):
    publication_date = ''
    try:
...
def get_online_resource(url, name, function='download'):
...
# TESTS
# package_list = get_metadata_list_with_resources()
# # test_package_0 = package_list[45]
# # convert_iso(test_package_0)
# test_package = package_list[100]
# convert_iso(test_package)
